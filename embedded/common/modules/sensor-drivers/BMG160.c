...
static void gyro_get_chip_id(uint8_t *chipId)
{
    if ( p_bmg160 == NULL ) 
        return;

	/*Read CHIP_ID */
    p_bmg160->BMG160_BUS_READ_FUNC(p_bmg160->dev_addr,	\
						BMG160_CHIP_ID_ADDR, chipId, 1);
}

static void gyro_activate(bool enable)
{
	if (enable) {
		bmg160_set_mode(BMG160_MODE_NORMAL);
		bmg160_set_int_od(BMG160_INT1, 0);
		bmg160_set_int_lvl(BMG160_INT1, 1);
		bmg160_set_int_data(BMG160_INT1_DATA, 1);
		bmg160_set_latch_int(7);
		...
		NVIC_ClearPendingIRQ(GYRO_PINT_IRQn);
	}
	else {
		bmg160_set_data_en(0);
		NVIC_DisableIRQ(GYRO_PINT_IRQn);
		bmg160_set_mode(BMG160_MODE_FASTPOWERUP);		
...
void Gyro_Initialize(void)
{
    ...
    /* Reset all registers default setting */
    // QLY: Comment from bosch that doing a software reset cause SDA stay low forever! 
//    bmg160_set_soft_reset();
//    dev_i2c_delay(200);

    /* Get CHIP ID */
    gyro_get_chip_id( &bmg160.chip_id);

    D0_printf("Gyro Chip ID = 0x%x\r\n", bmg160.chip_id);
 
	dev_i2c_delay(10);
	bmg160_set_mode(BMG160_MODE_NORMAL);
	//dev_i2c_delay(10);
	/* gyro_SetInterval(pSens,10); */	
    bmg160_set_bw(C_BMG160_BW_32Hz_U8X);    // BW = 32Hz, ODR = 100Hz

	//dev_i2c_delay(200);
	bmg160_set_range_reg(C_BMG160_Zero_U8X);	/* Zero = 2000 dps */
	//dev_i2c_delay(200);
...
void Gyro_ConfigDataInt(osp_bool_t enable)
{
	if (enable) {
		bmg160_set_mode(BMG160_MODE_NORMAL);
		bmg160_set_int_od(BMG160_INT1, 0);
...
void Gyro_ReadData(MsgGyroData *gyroData)
{
	struct bmg160_data_t data;

	/* Uses Bosch library functions for sensor data read */
	/* ##KW## Read counts or diffs? */
	bmg160_get_dataXYZ(&data);

	if (gyroData) {
		gyroData->X = (int16_t) data.datax;
		gyroData->Y = (int16_t) data.datay;
		gyroData->Z = (int16_t) data.dataz;
	}
}

...
static void accel_activate(bool enable)
{
	if (enable) {
		bma2x2_set_mode(BMA2x2_MODE_NORMAL);		// Normal power
		bma2x2_set_range(BMA2x2_RANGE_4G);			/* set range 4g for grange */
		bma2x2_set_bandwidth(BMA2x2_BW_31_25HZ);

		/* configure interrupt pin INT1 as data interrupt */ 
		bma2x2_set_newdata(BMA2x2_INT1_NDATA, 1);
		bma2x2_set_int_lvl(BMA2x2_INT1_LEVEL, 1);
		bma2x2_set_int_od(BMA2x2_INT1_OUTPUT, 0);
		bma2x2_set_latch_int(BMA2x2_LATCH_DUR_LATCH);
		/* enable data interrupt */
		bma2x2_set_Int_Enable(BMA2x2_DATA_EN, 1);
		/* configure INT2 pin */
		bma2x2_set_int_lvl(BMA2x2_INT2_LEVEL, 1);
		bma2x2_set_int_od(BMA2x2_INT2_OUTPUT, 0);
		...
		bma2x2_set_mode(BMA2x2_MODE_DEEP_SUSPEND);		// Low power
	}
}
...
void Accel_Initialize(AccelInitOption option)
{
	...
	/* accel_setDelay(pSens, 16); */
	bma2x2_set_bandwidth(BMA2x2_BW_62_50HZ);
    ...
    // QLY: Somehow on the LPC54102 platform, no accel interrupt if the following API is not call with argument 'true' 
	accel_activate(true);     
}

void Accel_ConfigDataInt(osp_bool_t enable)
{
	if (enable) {
		bma2x2_set_mode(BMA2x2_MODE_NORMAL);		// Normal power
		bma2x2_set_range(BMA2x2_RANGE_4G);			/* set range 4g for grange */
		/* Baccel_setDelay(pSens, 16); */
...
void Accel_ReadData(MsgAccelData *accelData )
{
	bma2x2acc_t bma;
	bma2x2_read_accel_xyz(&bma);	// get acceleration data from sensor	
	if (accelData) {
		accelData->X = bma.x;
		accelData->Y = bma.y;
		accelData->Z = bma.z;

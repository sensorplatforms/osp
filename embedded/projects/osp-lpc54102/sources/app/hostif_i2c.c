...
void I2C_HOSTIF_IRQHandler(void)
{
   ...
   {
        uint8_t ret;
       /* D0_printf("   bytesReceived = %u, rxCount = %d, txCount = %d\r\n", 
               *((uint8_t *)slave_i2c_handle.pXfer.rxBuff), slave_i2c_handle.pXfer.rxSz, 0); */
   ...
   if ( slave_i2c_handle.i2c_operation == 0x2 ) 
   {
       ...
       //D0_printf("   bytesSent = %d\r\n", slave_i2c_handle.pXfer.bytesSent);
...
void Hostif_StartTx(uint8_t *pBuf, uint16_t size, int magic)
{
    ...
    /* replace g_hostif.txBuff with i2c_write API  
         ...
         */
   //D0_printf("1. Startx write = %x,new data = %x\r\n",slave_i2c_handle.pXfer.txBuff,pBuf);
                     ...
                     uint8_t *pBuf_next, uint16_t size_next, int magic)
{
    ...
    /* No actual transmission happens here, but the I2c HAL keeps the buffer for transmission
        * when the master is actually ready to receive. 
        */
    //D0_printf("3.Startx write = %x,new data = %x\r\n",
               //slave_i2c_handle.pXfer.txBuff,i2cXfer.txBuff);
    i2c_slave_write(&slave_i2c_handle,(const char *)pBuf,size-1);
...
{     
   ...
   /* reset host IF control data structure */
    ...
    sda = ENCODE_PORT_PIN((uint8_t)Port_0, (uint8_t)Pin_27);
    scl = ENCODE_PORT_PIN((uint8_t)Port_0, (uint8_t)Pin_28);
    /* I2C slave initialisation */
    i2c_init(&slave_i2c_handle,sda,scl);

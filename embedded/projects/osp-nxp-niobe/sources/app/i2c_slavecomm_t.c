...
static volatile uint8_t ClearQueue = 0;
...
static void SensorEnable(ASensorType_t sen)
{
	...
	SensorState[0] |= 1 << A_SENSOR_SIGNIFICANT_MOTION;
...
static void SendSensorBoolData(uint8_t sensorId, MsgSensorBoolData *pMsg)
{
	Buffer_t	*pHifPacket;
	uint8_t		*pPayload;
	...
	SensorHubAssertInt();	/* Assert interrupt */
...
ASF_TASK void I2CCommTask(ASF_TASK_ARG)
{
	...
	while(1) {
		...
		switch (rcvMsg->msgId) {
		case MSG_ACC_DATA:
			SendSensorData(SENSOR_ACCELEROMETER_UNCALIBRATED,
		...
		case MSG_CAL_ACC_DATA:
			SendSensorData(SENSOR_ACCELEROMETER_CALIBRATED,
					&rcvMsg->msg.msgAccelData);
			break;
		case MSG_CAL_MAG_DATA:
			SendSensorData(SENSOR_MAGNETIC_FIELD_CALIBRATED,
					&rcvMsg->msg.msgMagData);
			break;
		case MSG_CAL_GYRO_DATA:
			SendSensorData(SENSOR_GYROSCOPE_CALIBRATED,
					&rcvMsg->msg.msgGyroData);
			break;

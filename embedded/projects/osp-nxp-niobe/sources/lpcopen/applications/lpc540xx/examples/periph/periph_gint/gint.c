...
#endif /* defined(BOARD_NXP_LPCXPRESSO_54000) */
...
int main(void)
{
	...
	Chip_GPIOGP_SelectLowLevel(LPC_GINT, 0, BUTTON1_PORT, (uint32_t) 1 << BUTTON1_PIN);
	Chip_GPIOGP_EnableGroupPins(LPC_GINT, 0, BUTTON1_PORT, (uint32_t) 1 << BUTTON1_PIN);
	/* Button 2 Setup for falling edge*/
	Chip_GPIOGP_SelectLowLevel(LPC_GINT, 0, BUTTON2_PORT, (uint32_t) 1 << BUTTON2_PIN);
	Chip_GPIOGP_EnableGroupPins(LPC_GINT, 0, BUTTON2_PORT, (uint32_t) 1 << BUTTON2_PIN);

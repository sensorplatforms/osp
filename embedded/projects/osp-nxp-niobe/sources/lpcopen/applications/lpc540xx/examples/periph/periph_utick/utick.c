...
int main(void)
{
	...
	Chip_UTICK_ClearInterrupt(LPC_UTICK);
	/* Set the UTick for a delay of 1s and in repeat mode */
	Chip_UTICK_SetTick(LPC_UTICK, Chip_Clock_GetWDTOSCRate(), true);
	/* Enable Wake up from deep sleep mode due to UTick */
	Chip_SYSCTL_EnableWakeup(STARTERP0_UTICK);

	/* Enable UTICK interrupt */
	NVIC_EnableIRQ(UTICK_IRQn);

	while (1) {
		// FIXME - Use ROM API's to put chip to deep sleep mode
		__WFI();
	}

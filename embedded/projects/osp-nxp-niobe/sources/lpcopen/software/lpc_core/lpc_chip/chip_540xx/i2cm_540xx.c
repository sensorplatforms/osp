...
void Chip_I2CM_SetBusSpeed(LPC_I2C_T *pI2C, uint32_t busSpeed)
{
	...
	uint32_t scl = Chip_Clock_GetAsyncSysconClockRate() / (Chip_I2C_GetClockDiv(pI2C) * busSpeed);
...
{
	/* Temporary workaround for I2CM state machine reset */
	volatile uint32_t reg;

	/* Workaround: I2C master appears to hang if this isn't done.
     Some correlation with I2C divider. Needs check. */
	reg = pI2C->CFG & I2C_CFG_MASK;
	reg &= ~I2C_CFG_MSTEN;
	pI2C->CFG = reg;
	reg |= I2C_CFG_MSTEN;
	pI2C->CFG = reg;
